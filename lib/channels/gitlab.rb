...
service 'gitlab' do
  ...
  listener 'push' do
    ...
    start do |data|
      ...
      begin
        gitlab = Gitlab.client(endpoint:endpoint, private_token:token, user_agent:'Factor.io Agent')
        all_projects = []
        page=0
        begin
      ...
      hook_url = web_hook id:'post_receive' do
        start do |listener_start_params,data,req,res|
          ...
          if received_branch != branch
            ...
            begin
              project = gitlab.projects.select{|project| project.path_with_namespace == "#{username}/#{repo}"}.first
            rescue
